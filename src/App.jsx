...
const App = () => {
  ...
  const[settings, setSettings] = useState({
    ...
    skew:[0, 20], magnetic:[0, 0.2], tilt:[0, 40], tiltCard:[0, 25], explicit: false
  });
  ...
  const copyCode = () => {
    const code = mode === "text"
        ? `<ProximityText text="${settings.text}" splitBy="${settings.splitBy}" preset="${generatedPresetString}" config={${JSON.stringify(config, null, 2)}} />`
  ...
  return (
    <main style={{ backgroundColor: "#000", minHeight: "100vh", color: "#e0e0e0", fontFamily: "'Georama', sans-serif", padding: "40px 20px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ width: "100%", maxWidth: "1000px", minHeight: "500px", backgroundColor: "#070707", borderRadius: "24px", border: "1px solid #151515", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", marginBottom: "30px", boxShadow: "0 30px 60px rgba(0,0,0,0.5)", overflow: "hidden" }}>
        ...
        {mode === "text" ? (
          <ProximityText key={settings.text + settings.splitBy + generatedPresetString + JSON.stringify(config)} text={settings.text.split("\\n").join("\n")} splitBy={settings.splitBy} preset={generatedPresetString} reach={settings.reach} falloff={settings.falloff} config={config} style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: `${settings.fontSize}rem`, fontWeight: "800", textAlign: "center" }} />
        ) : (
          <Proximity key={"elements" + settings.elementCount + generatedPresetString + JSON.stringify(config)} selector=".dock-icon" preset={generatedPresetString} reach={settings.reach} falloff={settings.falloff} config={config} style={{ width: "100%", height: "100%", display: "flex", gap: "15px", alignItems: "center", justifyContent: "center", padding: "20px", flexWrap: "wrap" }}>
      ...
      <div style={{ width: "100%", maxWidth: "1000px", borderTop: "1px solid #1a1a1a", paddingTop: "60px", paddingBottom: "100px" }}>
        ...
        <DocSection icon={Fingerprint} title="Magnetic Pull" description="Elements physically move toward cursor." code={'<Proximity preset="magnetic" />'}><Proximity selector=".mag-box" preset="magnetic" config={{ magnetic: [0, 0.4] }} style={{ display: "flex", alignItems: "center", justifyContent: "center" }}><div className="mag-box" style={{ width: "80px", height: "80px", backgroundColor: "#fff", borderRadius: "50%" }} /></Proximity></DocSection>
        ...
        <DocSection icon={Wand2} title="Escape Hatch" description="Custom logic using intensity & coordinates." code={'onCalculate={(intensity, dist, dx, dy) => ({ scale: intensity })}'}><Proximity selector=".morph-box" reach={1.5} onCalculate={(intensity) => ({ borderRadius: `${intensity * 50}%`, backgroundColor: `rgba(16, 185, 129, ${Math.max(0.1, intensity)})`, scale: 1 + (intensity * 0.5) })} onReset={() => ({ borderRadius: "10%", backgroundColor: "rgba(255,255,255,0.1)", scale: 1 })} style={{ display: "flex", alignItems: "center", justifyContent: "center" }}><div className="morph-box" style={{ width: "120px", height: "120px", backgroundColor: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: "10%", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: "bold" }}>Morph</div></Proximity></DocSection>
      </div>
    </main>
  );
};
